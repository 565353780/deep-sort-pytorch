...
class OSNet(nn.Module):
    ...
    def __init__(self, num_classes, blocks, layers, channels, feature_dim=512, loss='softmax', IN=False, **kwargs):
        ...
        self.classifier = nn.Linear(self.feature_dim, 4101)
...
if __name__ == '__main__':
    ...
    model = osnet_ibn_x1_0(pretrained=False)
    # model = model.cuda()

    # batch_size = 1

    state_dict = torch.load('../../models/osnet_ibn_x1_0_msmt17_combineall_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip_jitter.pth')
    model.load_state_dict(state_dict)
    model.eval()

    x = torch.randn(1, 3, 256, 128, requires_grad=True)
